const { ApplicationCommandOptionType, ActionRowBuilder, ButtonBuilder, ButtonStyle, Client, EmbedBuilder, Interaction, PermissionFlagsBits } = require('discord.js');
...
module.exports = {
  ...
  callback: async (client, interaction) => {
    ...
    if (!token) {
      ...
      console.log(`\x1b[31m[${getTimestamp()}][DENIED ACCESS]:\x1b[0m ${interaction.user.tag} tried to run the command /gpublish but they do not have access to this command.`);
      return;
    }
    ...
    const CreateGame = async (csrf) => {
      try {
        ...
        const response = await axios.post(`https://apis.roblox.com/universes/v1/universes/create?groupId=${interaction.options.getString('group-id')}`, body, {
          headers: {
            'Cookie': `.ROBLOSECURITY=${interaction.options.getString('cookie')}`,
            'x-csrf-token': csrf,
        ...
        console.log(`\x1b[32m[${getTimestamp()}][GROUP PUBLISHER]:\x1b[0m Game created! (gameId: ${response.data.rootPlaceId})`);
        return response;
      } catch (error) {
        const errorEmbed = new EmbedBuilder()
        .setTitle('Error')
    ...
    const uploadGame = async (csrf, placeId, success, failed, loading, universeId) => {
      const response = await axios.post(`https://data.roblox.com/Data/Upload.ashx?assetid=${placeId}`, fs.readFileSync(newGameFile), {
        ...
        console.log(`\x1b[32m[${getTimestamp()}][GROUP PUBLISHER]:\x1b[0m Game uploaded! (gameId: ${placeId})`);
     ...
     const sendDM = async (success, failed, placeId, universeId) => {
      try {
        ...
        console.log(`\x1b[32m[${getTimestamp()}][GROUP PUBLISHER]:\x1b[0m Sent DM to user!`);
      } catch (error) {
        console.log(`\x1b[31m[${getTimestamp()}][GROUP PUBLISHER ERROR]:\x1b[0m Failed to send DM to user: ${error.message}`);
        interaction.editReply({ embeds: [failed] });
      }
    }
    ...
    const sendRequests = async () => {
      try {
        ...
        const newRequestSuccess = new UploadRequests({ requestId: requestId, gameURL: `https://www.roblox.com/games/${placeId}`, pubType: `Group Publishing (ID: ${interaction.options.getString('group-id')})`, userId: interaction.user.id, status: 'success' });
        ...
        fs.unlink(newGameFile, (err) => {
          if (err) throw err;
          console.log(`\x1b[32m[${getTimestamp()}][GROUP PUBLISHER]:\x1b[0m Deleted renamed file! (file: ${newGameFile})`);
        })
        ...
        .setTimestamp();
        interaction.editReply({ embeds: [errorEmbed], ephemeral: true });
        const newRequestFailed = new UploadRequests({ requestId: requestId, gameURL: `NaN`, pubType: `Group Publishing (ID: ${interaction.options.getString('group-id')})`, userId: interaction.user.id, status: 'failed' });
    ...
    // Final requests and interactions
      sendRequests();
      console.log(`\x1b[32m[${getTimestamp()}][GROUP PUBLISHER]:\x1b[0m ${interaction.user.tag} has published a game using our theme ${interaction.options.getString('games')}!`)
    } catch (error) {
      const errorEmbed = new EmbedBuilder()
        .setTitle('Error')
  ...
  name: 'gpublish',
    description: 'Group publisher best suited for cellphone users.',
    options: [
      {
        name: 'games',
      ...
      {
        name: 'group-id',
        description: 'ID of the group you want to use for publishing.',
        type: ApplicationCommandOptionType.String,
        required: true,
      },
      {
